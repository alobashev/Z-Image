...
class ZImageTransformer2DModel(nn.Module):
    ...
    def forward(self, x, t, cap_feats, patch_size=2, f_patch_size=1):
        ...
        print("\n\nAnalyzing layer10")
        print(f"\t\t>>>unified_state.shape={unified_state.shape}, unified_state.dtype={unified_state.dtype}")
        # layer10
        layer10_mod = torch.matmul(adaln_input, self.layer10_adaln_weight.t()) + self.layer10_adaln_bias
        print("layer10_mod = torch.matmul(adaln_input, self.layer10_adaln_weight.t()) + self.layer10_adaln_bias")
        print(f"\t\t>>>layer10_mod.shape={layer10_mod.shape}, layer10_mod.dtype={layer10_mod.dtype}")
        layer10_mod = layer10_mod.unsqueeze(1)
        print("layer10_mod = layer10_mod.unsqueeze(1)")
        print(f"\t\t>>>layer10_mod.shape={layer10_mod.shape}, layer10_mod.dtype={layer10_mod.dtype}")
        layer10_scale_msa, layer10_gate_msa, layer10_scale_mlp, layer10_gate_mlp = layer10_mod.chunk(4, dim=2)
        print("layer10_scale_msa, layer10_gate_msa, layer10_scale_mlp, layer10_gate_mlp = layer10_mod.chunk(4, dim=2)")
        layer10_gate_msa = torch.tanh(layer10_gate_msa)
        print("layer10_gate_msa = torch.tanh(layer10_gate_msa)")
        print(f"\t\t>>>layer10_gate_msa.shape={layer10_gate_msa.shape}, layer10_gate_msa.dtype={layer10_gate_msa.dtype}")
        layer10_gate_mlp = torch.tanh(layer10_gate_mlp)
        print("layer10_gate_mlp = torch.tanh(layer10_gate_mlp)")
        print(f"\t\t>>>layer10_gate_mlp.shape={layer10_gate_mlp.shape}, layer10_gate_mlp.dtype={layer10_gate_mlp.dtype}")
        layer10_scale_msa = 1.0 + layer10_scale_msa
        print("layer10_scale_msa = 1.0 + layer10_scale_msa")
        print(f"\t\t>>>layer10_scale_msa.shape={layer10_scale_msa.shape}, layer10_scale_msa.dtype={layer10_scale_msa.dtype}")
        layer10_scale_mlp = 1.0 + layer10_scale_mlp
        print("layer10_scale_mlp = 1.0 + layer10_scale_mlp")
        print(f"\t\t>>>layer10_scale_mlp.shape={layer10_scale_mlp.shape}, layer10_scale_mlp.dtype={layer10_scale_mlp.dtype}")
        ...
        print("layer10_attn_in = unified_state * torch.rsqrt(unified_state.pow(2).mean(-1, keepdim=True) + self.norm_eps) * self.layer10_attention_norm1_weight")
        print(f"\t\t>>>layer10_attn_in.shape={layer10_attn_in.shape}, layer10_attn_in.dtype={layer10_attn_in.dtype}")
        layer10_attn_in = layer10_attn_in * layer10_scale_msa
        print("layer10_attn_in = layer10_attn_in * layer10_scale_msa")
        print(f"\t\t>>>layer10_attn_in.shape={layer10_attn_in.shape}, layer10_attn_in.dtype={layer10_attn_in.dtype}")
        layer10_q = torch.matmul(layer10_attn_in, self.layer10_attention_to_q_weight.t())
        print("layer10_q = torch.matmul(layer10_attn_in, self.layer10_attention_to_q_weight.t())")
        print(f"\t\t>>>layer10_q.shape={layer10_q.shape}, layer10_q.dtype={layer10_q.dtype}")
        layer10_k = torch.matmul(layer10_attn_in, self.layer10_attention_to_k_weight.t())
        print("layer10_k = torch.matmul(layer10_attn_in, self.layer10_attention_to_k_weight.t())")
        print(f"\t\t>>>layer10_k.shape={layer10_k.shape}, layer10_k.dtype={layer10_k.dtype}")
        layer10_v = torch.matmul(layer10_attn_in, self.layer10_attention_to_v_weight.t())
        print("layer10_v = torch.matmul(layer10_attn_in, self.layer10_attention_to_v_weight.t())")
        print(f"\t\t>>>layer10_v.shape={layer10_v.shape}, layer10_v.dtype={layer10_v.dtype}")
        layer10_q = layer10_q.view(1, unified_state.shape[1], self.n_heads, self.head_dim)
        print("layer10_q = layer10_q.view(1, unified_state.shape[1], self.n_heads, self.head_dim)")
        print(f"\t\t>>>layer10_q.shape={layer10_q.shape}, layer10_q.dtype={layer10_q.dtype}")
        layer10_k = layer10_k.view(1, unified_state.shape[1], self.n_kv_heads, self.head_dim)
        print("layer10_k = layer10_k.view(1, unified_state.shape[1], self.n_kv_heads, self.head_dim)")
        print(f"\t\t>>>layer10_k.shape={layer10_k.shape}, layer10_k.dtype={layer10_k.dtype}")
        layer10_v = layer10_v.view(1, unified_state.shape[1], self.n_kv_heads, self.head_dim)
        print("layer10_v = layer10_v.view(1, unified_state.shape[1], self.n_kv_heads, self.head_dim)")
        print(f"\t\t>>>layer10_v.shape={layer10_v.shape}, layer10_v.dtype={layer10_v.dtype}")
        ...
        print("layer10_q = layer10_q * torch.rsqrt(layer10_q.pow(2).mean(-1, keepdim=True) + self.norm_eps) * self.layer10_norm_q_weight.view(1, 1, 1, -1)")
        print(f"\t\t>>>layer10_q.shape={layer10_q.shape}, layer10_q.dtype={layer10_q.dtype}")
        ...
        print("layer10_k = layer10_k * torch.rsqrt(layer10_k.pow(2).mean(-1, keepdim=True) + self.norm_eps) * self.layer10_norm_k_weight.view(1, 1, 1, -1)")
        print(f"\t\t>>>layer10_k.shape={layer10_k.shape}, layer10_k.dtype={layer10_k.dtype}")
        layer10_freqs_cos = unified_freqs.real.unsqueeze(2).to(layer10_attn_in.dtype)
        print("layer10_freqs_cos = unified_freqs.real.unsqueeze(2).to(layer10_attn_in.dtype)")
        print(f"\t\t>>>layer10_freqs_cos.shape={layer10_freqs_cos.shape}, layer10_freqs_cos.dtype={layer10_freqs_cos.dtype}")
        layer10_freqs_sin = unified_freqs.imag.unsqueeze(2).to(layer10_attn_in.dtype)
        print("layer10_freqs_sin = unified_freqs.imag.unsqueeze(2).to(layer10_attn_in.dtype)")
        print(f"\t\t>>>layer10_freqs_sin.shape={layer10_freqs_sin.shape}, layer10_freqs_sin.dtype={layer10_freqs_sin.dtype}")
        layer10_q_even = layer10_q[..., 0::2]
        print("layer10_q_even = layer10_q[..., 0::2]")
        print(f"\t\t>>>layer10_q_even.shape={layer10_q_even.shape}, layer10_q_even.dtype={layer10_q_even.dtype}")
        layer10_q_odd = layer10_q[..., 1::2]
        print("layer10_q_odd = layer10_q[..., 1::2]")
        print(f"\t\t>>>layer10_q_odd.shape={layer10_q_odd.shape}, layer10_q_odd.dtype={layer10_q_odd.dtype}")
        ...
        print("layer10_q = torch.stack([layer10_q_even * layer10_freqs_cos - layer10_q_odd * layer10_freqs_sin, layer10_q_even * layer10_freqs_sin + layer10_q_odd * layer10_freqs_cos], dim=-1).flatten(3)")
        print(f"\t\t>>>layer10_q.shape={layer10_q.shape}, layer10_q.dtype={layer10_q.dtype}")
        layer10_k_even = layer10_k[..., 0::2]
        print("layer10_k_even = layer10_k[..., 0::2]")
        print(f"\t\t>>>layer10_k_even.shape={layer10_k_even.shape}, layer10_k_even.dtype={layer10_k_even.dtype}")
        layer10_k_odd = layer10_k[..., 1::2]
        print("layer10_k_odd = layer10_k[..., 1::2]")
        print(f"\t\t>>>layer10_k_odd.shape={layer10_k_odd.shape}, layer10_k_odd.dtype={layer10_k_odd.dtype}")
        ...
        print("layer10_k = torch.stack([layer10_k_even * layer10_freqs_cos - layer10_k_odd * layer10_freqs_sin, layer10_k_even * layer10_freqs_sin + layer10_k_odd * layer10_freqs_cos], dim=-1).flatten(3)")
        print(f"\t\t>>>layer10_k.shape={layer10_k.shape}, layer10_k.dtype={layer10_k.dtype}")
        layer10_q = layer10_q.transpose(1, 2)
        print("layer10_q = layer10_q.transpose(1, 2)")
        print(f"\t\t>>>layer10_q.shape={layer10_q.shape}, layer10_q.dtype={layer10_q.dtype}")
        layer10_k = layer10_k.transpose(1, 2)
        print("layer10_k = layer10_k.transpose(1, 2)")
        print(f"\t\t>>>layer10_k.shape={layer10_k.shape}, layer10_k.dtype={layer10_k.dtype}")
        layer10_v = layer10_v.transpose(1, 2)
        print("layer10_v = layer10_v.transpose(1, 2)")
        print(f"\t\t>>>layer10_v.shape={layer10_v.shape}, layer10_v.dtype={layer10_v.dtype}")



        # ---------------------------------------------------
        # FP16-only online softmax attention (no fp32) begin
        # ---------------------------------------------------
        q = layer10_q
        k = layer10_k
        v = layer10_v
        ...
        # scale in compute dtype (avoid fp32 constant propagation)
        scale = torch.tensor(self.attention_scale, device=q.device, dtype=q.dtype)
        
        # Running state per (B,H,query_index)
        m  = torch.full((B, H, N, 1), torch.finfo(q.dtype).min, device=q.device, dtype=q.dtype)
        l  = torch.zeros((B, H, N, 1), device=q.device, dtype=q.dtype)
        lc = torch.zeros_like(l)  # compensation for l (Kahan-like, per block)
        
        o  = torch.zeros((B, H, N, Dv), device=q.device, dtype=q.dtype)
        oc = torch.zeros_like(o)  # compensation for o (Kahan-like, per block)
        
        # Stream over keys/values in blocks to avoid materializing [N,N]
        BLOCK_K = 128  # tune for your chip (64/128/256)
        
        for start in range(0, N, BLOCK_K):
            end = min(start + BLOCK_K, N)
        
            k_b = k[:, :, start:end, :]                 # [B,H,Bk,D]
            v_b = v[:, :, start:end, :]                 # [B,H,Bk,Dv]
        
            # logits for this block: s = (q @ k_b^T) * scale
            # s: [B,H,N,Bk]
            s = torch.matmul(q, k_b.transpose(-2, -1)) * scale
        
            # block max, then new running max
            mb = s.max(dim=-1, keepdim=True).values     # [B,H,N,1]
            m_new = torch.maximum(m, mb)                # [B,H,N,1]
        
            # rescale old accumulators by r = exp(m - m_new) in (0,1]
            r = torch.exp(m - m_new)                    # [B,H,N,1]
            ...
            # u = exp(s - m_new) is safe (<= 1)
            u = torch.exp(s - m_new)                    # [B,H,N,Bk]
        
            # sum_u = sum(u) (keep dtype)
            sum_u = u.sum(dim=-1, keepdim=True, dtype=q.dtype)  # [B,H,N,1]
        
            # Kahan-like update for l (per block)
            y  = sum_u - lc
            t  = l + y
            lc = (t - l) - y
            l  = t
        
            # block contribution to output: u @ v_b
            # [B,H,N,Bk] @ [B,H,Bk,Dv] -> [B,H,N,Dv]
            block_out = torch.matmul(u, v_b)
        
            # Kahan-like update for o (per block, vector-valued)
            y2 = block_out - oc
            t2 = o + y2
            oc = (t2 - o) - y2
            o  = t2
        ...
        # Combine compensated sums
        l_total = l + lc            # [B,H,N,1]
        o_total = o + oc            # [B,H,N,Dv]
        
        # -------------------------------
        # Reciprocal via Newton (unrolled)
        # -------------------------------
        inv_l = torch.reciprocal(l_total)               # initial guess in FP16
        two   = torch.tensor(2.0, device=q.device, dtype=q.dtype)
        
        inv_l = inv_l * (two - l_total * inv_l)         # Newton step 1
        inv_l = inv_l * (two - l_total * inv_l)         # Newton step 2
        inv_l = inv_l * (two - l_total * inv_l)         # Newton step 3 (optional but helps)
        
        # Final attention: o / l
        layer10_attn = o_total * inv_l                  # [B,H,N,Dv]
        # ---------------------------------------------------
        # FP16-only online softmax attention (no fp32) end
        # ---------------------------------------------------



        # OLD implementation with recasting to fp32
        
        # layer10_scores = torch.matmul(layer10_q.float(), layer10_k.float().transpose(-2, -1)) * self.attention_scale
        # print("layer10_scores = torch.matmul(layer10_q.float(), layer10_k.float().transpose(-2, -1)) * self.attention_scale")
        # print(f"\t\t>>>layer10_scores.shape={layer10_scores.shape}, layer10_scores.dtype={layer10_scores.dtype}")



        
        # layer10_probs = torch.softmax(layer10_scores, dim=-1).to(layer10_v.dtype)
        # print("layer10_probs = torch.softmax(layer10_scores, dim=-1).to(layer10_v.dtype)")
        # print(f"\t\t>>>layer10_probs.shape={layer10_probs.shape}, layer10_probs.dtype={layer10_probs.dtype}")

        
        # layer10_attn = torch.matmul(layer10_probs, layer10_v)
        # print("layer10_attn = torch.matmul(layer10_probs, layer10_v)")
        # print(f"\t\t>>>layer10_attn.shape={layer10_attn.shape}, layer10_attn.dtype={layer10_attn.dtype}")



        
        layer10_attn = layer10_attn.transpose(1, 2).reshape(1, unified_state.shape[1], self.q_proj_dim)
        print("layer10_attn = layer10_attn.transpose(1, 2).reshape(1, unified_state.shape[1], self.q_proj_dim)")
        print(f"\t\t>>>layer10_attn.shape={layer10_attn.shape}, layer10_attn.dtype={layer10_attn.dtype}")
        layer10_attn = torch.matmul(layer10_attn, self.layer10_attention_to_out_weight.t())
        print("layer10_attn = torch.matmul(layer10_attn, self.layer10_attention_to_out_weight.t())")
        print(f"\t\t>>>layer10_attn.shape={layer10_attn.shape}, layer10_attn.dtype={layer10_attn.dtype}")
        ...
        print("layer10_attn = layer10_attn * torch.rsqrt(layer10_attn.pow(2).mean(-1, keepdim=True) + self.norm_eps) * self.layer10_attention_norm2_weight")
        print(f"\t\t>>>layer10_attn.shape={layer10_attn.shape}, layer10_attn.dtype={layer10_attn.dtype}")
        unified_state = unified_state + layer10_gate_msa * layer10_attn
        print("unified_state = unified_state + layer10_gate_msa * layer10_attn")
        print(f"\t\t>>>unified_state.shape={unified_state.shape}, unified_state.dtype={unified_state.dtype}")
        ...
        print("layer10_ffn_in = unified_state * torch.rsqrt(unified_state.pow(2).mean(-1, keepdim=True) + self.norm_eps) * self.layer10_ffn_norm1_weight")
        print(f"\t\t>>>layer10_ffn_in.shape={layer10_ffn_in.shape}, layer10_ffn_in.dtype={layer10_ffn_in.dtype}")
        layer10_ffn_in = layer10_ffn_in * layer10_scale_mlp
        print("layer10_ffn_in = layer10_ffn_in * layer10_scale_mlp")
        print(f"\t\t>>>layer10_ffn_in.shape={layer10_ffn_in.shape}, layer10_ffn_in.dtype={layer10_ffn_in.dtype}")
        layer10_ffn_a = torch.matmul(layer10_ffn_in, self.layer10_feed_forward_w1_weight.t())
        print("layer10_ffn_a = torch.matmul(layer10_ffn_in, self.layer10_feed_forward_w1_weight.t())")
        print(f"\t\t>>>layer10_ffn_a.shape={layer10_ffn_a.shape}, layer10_ffn_a.dtype={layer10_ffn_a.dtype}")
        layer10_ffn_b = torch.matmul(layer10_ffn_in, self.layer10_feed_forward_w3_weight.t())
        print("layer10_ffn_b = torch.matmul(layer10_ffn_in, self.layer10_feed_forward_w3_weight.t())")
        print(f"\t\t>>>layer10_ffn_b.shape={layer10_ffn_b.shape}, layer10_ffn_b.dtype={layer10_ffn_b.dtype}")
        layer10_ffn = F.silu(layer10_ffn_a) * layer10_ffn_b
        print("layer10_ffn = F.silu(layer10_ffn_a) * layer10_ffn")
        print(f"\t\t>>>layer10_ffn.shape={layer10_ffn.shape}, layer10_ffn.dtype={unified_state.dtype}")
        layer10_ffn = torch.matmul(layer10_ffn, self.layer10_feed_forward_w2_weight.t())
        print("layer10_ffn = torch.matmul(layer10_ffn, self.layer10_feed_forward_w2_weight.t())")
        print(f"\t\t>>>layer10_ffn.shape={layer10_ffn.shape}, layer10_ffn.dtype={layer10_ffn.dtype}")
        layer10_ffn = layer10_ffn * torch.rsqrt(layer10_ffn.pow(2).mean(-1, keepdim=True) + self.norm_eps) * self.layer10_ffn_norm2_weight
        print("layer10_ffn = layer10_ffn * torch.rsqrt(layer10_ffn.pow(2).mean(-1, keepdim=True) + self.norm_eps) * self.layer10_ffn_norm2_weight")
        print(f"\t\t>>>layer10_ffn.shape={layer10_ffn.shape}, layer10_ffn.dtype={layer10_ffn.dtype}")
        unified_state = unified_state + layer10_gate_mlp * layer10_ffn
        print("unified_state = unified_state + layer10_gate_mlp * layer10_ffn")
        print(f"\t\t>>>unified_state.shape={unified_state.shape}, unified_state.dtype={unified_state.dtype}")
